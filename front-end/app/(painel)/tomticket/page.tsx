...
export default function TomticketPage() {
  ...
  // 2. FILTRAGEM DE DADOS DA TABELA
  ...
  // 4. FUNÇÃO EXPORTAR EXCEL
  const exportarExcel = () => {
    if (!dadosFiltrados || dadosFiltrados.length === 0) return;

    const dadosPlanilha = dadosFiltrados.map((u) => ({
    ...
    XLSX.utils.book_append_sheet(workbook, worksheet, "Métricas Tomticket");

    XLSX.writeFile(workbook, `relatorio_tomticket_${startDate}_a_${endDate}.xlsx`);
  };

  // 5. FUNÇÃO EXPORTAR PDF
  const exportarPDF = () => {
    if (!dadosFiltrados || dadosFiltrados.length === 0) return;
    ...
    doc.setFontSize(10);
    doc.text(`Período: ${startDate} até ${endDate} | Gerado em: ${new Date().toLocaleDateString("pt-BR")}`, 14, 22);
    ...
    const tableRows = dadosFiltrados.map((u) => [
      u.nome_usuario,
    ...
    doc.save(`relatorio_tomticket_${startDate}_a_${endDate}.pdf`);
  };
                      ...
                      <tr className="bg-zinc-950/90 border-b border-zinc-800">
                        <td colSpan={8} className="p-4 md:p-6">
                          <div className="flex flex-col gap-4">
                            ...
                            {/* Grid Principal com Alinhamento de Altura */}
                            <div className="grid grid-cols-1 lg:grid-cols-2 gap-4 items-stretch">

                              {/* COLUNA ESQUERDA: Clientes Atendidos */}
                              ...
                              {/* COLUNA DIREITA: Protocolos e Categorias */}
